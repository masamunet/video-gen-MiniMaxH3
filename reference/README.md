...
## video_minimax_h3_t2v-upscale.json (最新・こちらが正)

Latent Upscale の**バイパスを外した状態**の書き出し。アプリの
...
## video_minimax_h3_t2v-upscale-api.json (旧)

2026-08-17 時点の書き出し。Latent Upscale が**バイパス中**のため、ComfyUI の
API 書き出しの仕様でノード自体が現れず、パススルー後の結線
(`105:14.latent_image` = `["105:104", 1]`) だけが残っている。
アプリの `buildWorkflow({ upscale: false })` の出力がこれと同じ結線になる。

翻訳ノードが `145:142` `GFDeepTranslate` になっている点だけ新しい書き出しと異なる
(新しい方とアプリはどちらも `145:155` `GoogleTranslateTextNode`)。

いずれも元ファイルは `/Volumes/cola_HDD/win11/` にある。
