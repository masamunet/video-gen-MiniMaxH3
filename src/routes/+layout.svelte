<script lang="ts">
	...
	import { settings, bossMode } from '$lib/stores.svelte';
...
<div class="relative z-10 flex h-dvh flex-col">
	...
	>
		<!-- ロゴ: クリックでボスが来たモード ON (動画をグレー表示)。メニューの「生成」で解除 -->
		<a href="/" class="flex items-center gap-2.5" onclick={() => (bossMode.value = true)}>
		...
		<nav class="flex items-center gap-1">
			{#each nav as item (item.href)}
				...
				<a
					...
					onclick={() => {
						if (item.href === '/') bossMode.value = false;
					}}
				>
					<item.icon size={14} />
					{item.label}
